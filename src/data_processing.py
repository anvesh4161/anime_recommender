...
class DataProcessor:
    ...
    def encode_data(self):
        try:
            ...
            self.user2user_encoded = {x : i for i , x in enumerate(user_ids)} # Encoding the users IDs 11056 ---> 98 (user2user_encoded)
            self.user2user_decoded = {i : x for i , x in enumerate(user_ids)} # Decoding the users IDs 98 ---> 11056 (user2user_decoded)
            self.rating_df['user'] = self.rating_df['user_id'].map(self.user2user_encoded)

            ## Anime
            anime_ids = self.rating_df['anime_id'].unique().tolist()
            self.anime2anime_encoded = {x : i for i , x in enumerate(anime_ids)} # Encoding the anime IDs 11056 ---> 98 (anime2anime_encoded)
            self.anime2anime_decoded = {i : x for i , x in enumerate(anime_ids)} # Decoding the anime IDs 98 ---> 11056 (anime2anime_decoded)
    ...
    def save_artifacts(self):
        try:
            ...
            logger.info("All the training, testing data and rating_df is saved now ...")
        except Exception as e:
    ...
    def run(self):
        try:
            ...
            logger.info("Data Processing pipeline ran sucessfully.....CONGRATULATIONS!!!")
